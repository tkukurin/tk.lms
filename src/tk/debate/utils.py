...
class LocalModel:
  ...
  def __new__(cls, model_id: str = "gg-hf/gemma-2b-it", gpuclean: bool = True):
  ...
  def __call__(self, answer_context, **kw):
    ...
    out = self.gen(answer_context, max_new_tokens=512, **kw)
...
def generate_answer_gpt(answer_context) -> oai_types.ChatCompletion:
  try:
    ...
    create = ApiModel()
    completion = create(
        model="gpt-3.5-turbo-0125", messages=answer_context, n=1
    )
